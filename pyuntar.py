...
def untar(
    ...
    TarType = "tar"
    if (extract != True and extract):
        extract = False
    if (lsonly != True and lsonly):
        lsonly = False
    if (extract is True):
        lsonly = False
    if (compression == "gz"):
    ...
    if (compression == "bzip" or compression == "bz" or compression == "bz2"):
        compression = "bzip2"
    if (compression is not None and compression != "gzip" and compression !=
            "bzip2" and compression != "lzma" and compression != "xz"):
        compression = None
    if (compression == "gzip"):
        try:
            import gzip
    ...
    while (thandle.tell() < TarSizeEnd):
        ...
        thandle.seek(-57, 1)
        if (findfile is not None and FileType != "L" and re.search(
                "/" + qfindfile + "/", RealFileName)):
            thandle.seek(8, 1)
            thandle.seek(8, 1)
            thandle.seek(8, 1)
        ...
        if (findfile is None or FileType == "L" or re.search(
                "/" + qfindfile + "/", RealFileName)):
            ...
            if (FileType == "L" and FileSize > 0):
                ...
                if (TarType == "ustar"):
                    ...
                    FilenamePrefix = strip_text_from_file(thandle, 155)
                    thandle.seek(12, 1)
        if (((verbose is True and extract is True) or (
                verbose is False and lsonly is True)) and Checksum != 0):
        ...
        if (verbose is True and lsonly is True and Checksum != 0):
            ...
            log.info(permissionstr +
                     " " +
                     str(check_if_int_is_empty(OwnerID)) +
                     "/" +
                     str(check_if_int_is_empty(GroupID)) +
                     " " +
                     str(FileSize).rjust(15) +
                     " " +
                     datetime.datetime.utcfromtimestamp(check_if_int_is_empty(LastEdit)).strftime('%Y-%m-%d %H:%M') +
                     " " +
                     RealFileName)
        if ((findfile is None or re.search(
                "/" + qfindfile + "/", RealFileName)) and Checksum != 0):
            if (FileType == "0" or FileType == "7" or FileType == "g"):
                if (lsonly is True):
                    thandle.seek(FileSize, 1)
                if (lsonly is False):
            ...
            if (FileType == "0" or FileType == "1" or FileType ==
                    "2" or FileType == "5" or FileType == "7" or FileType == "g"):
                if (extract is False):
                    if (lsonly is True):
                        if (TarType == "tar"):
                            FileArray.update({i: {'FileName': FileName,
                                                  ...
                                                  'RealFileDirectory': RealFileDirectory,
                                                  'FileMode': FileMode,
                                                  'FileCHMOD': FileCHMOD,
                                                  'OwnerID': OwnerID,
                                                  'GroupID': GroupID,
                    ...
                    if (lsonly is False):
                        ...
                        if (TarType == "ustar"):
                            FileArray.update({i: {'FileName': FileName,
                                                  ...
                                                  'FilenamePrefix': FilenamePrefix,
                                                  'FileContent': FileContent}})
        if (extract is False and findfile is None and (
                i in FileArray and 'FileName' in FileArray[i]) and Checksum != 0):
            i += 1
        if (extract is False):
            if (findfile is not None and re.search("/" + qfindfile + "/", RealFileName)
                    and (i in FileArray and 'FileName' in FileArray[i]) and Checksum != 0):
                i += 1
        if (extract is True):
            if (findfile is not None and re.search(
                    "/" + qfindfile + "/", RealFileName) and Checksum != 0):
                i += 1
...
if __name__ == '__main__':
    ...
    if (getargs.list is False and getargs.extract is False):
        ...
        should_list = False
    untar(
        tarfile=getargs.tarfile,
        outdir=getargs.outputdir,
        chmod=getargs.chmod,
        extract=should_extract,
        lsonly=should_list,
        compression=getargs.decompress,
